...
try {
	_spawnChance = (_spawnChance * A3EAI_spawnChanceMultiplier);
	if (_spawnChance call A3EAI_chance) then {
...

		{
			if ((isPlayer _x) && {(typeOf _x) in [PLAYER_UNITS]}) exitWith {
				...
				if (
						(({if (_playerPos in _x) exitWith {1}} count ((nearestLocations [_playerPos,["A3EAI_BlacklistedArea","A3EAI_RandomSpawnArea"],1500]) - [_triggerLocation])) isEqualTo 0) && 
		...
		while {(_spawnPos isEqualTo []) && {_nearAttempts < 4}} do {
			...
			if (
				({if ((isPlayer _x) && {([eyePos _x,[(_spawnPosSelected select 0),(_spawnPosSelected select 1),(_spawnPosSelASL select 2) + 1.7],_x] call A3EAI_hasLOS) or ((_x distance _spawnPosSelected) < 145)}) exitWith {1}} count (_spawnPosSelected nearEntities [[PLAYER_UNITS,"LandVehicle"], 200]) isEqualTo 0) && 
		...
		if !(_spawnPos isEqualTo []) then {
			...
			throw format ["A3EAI Debug: Conditional checks failed for random spawn at %1. Canceling spawn.",(mapGridPosition _trigger)];
		};
	} else {
		throw format ["A3EAI Debug: Probability check failed for random spawn at %1. Canceling spawn.",(mapGridPosition _trigger)];
	};
} catch {
