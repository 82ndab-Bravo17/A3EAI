...
_nul = [] spawn {
	...
	diag_log "Debug: Waiting for HC player object setup to be completed.";
	
	waitUntil {uiSleep 3; A3EAI_HCPlayerLoggedIn};

	diag_log "Debug: HC player setup, creating HC unit.";
	...
	diag_log format ["Debug: Created HC unit %1",A3EAI_HCObject];
	
	diag_log "Attempting to connect to A3EAI server...";
	_versionKey = [configFile >> "CfgPatches" >> "A3EAI","A3EAIVersion","0.0.0"] call BIS_fnc_returnConfigEntry;
	...
	if (isNil "A3EAI_HC_serverResponse") exitWith {
		diag_log "Debug: Headless client connection timed out after 60 seconds of no response from server.";
		diag_log format ["HC Object Check: %1. Group: %2",A3EAI_HCObject,A3EAI_HCObjectGroup];
	};
	
	if !(A3EAI_HC_serverResponse) exitWith {diag_log "Debug: Headless client connection unsuccessful. HC authorization request rejected (incorrect HC version?).";};
	diag_log "Debug: Headless client connection successful. HC authorization request granted.";
...
_nul = [] spawn {
	...
	diag_log "Debug: Headless player object set up completed.";
	...
	while {true} do {
		{
			...
			//diag_log format ["Debug: Set headless player object variable %1 to %2.",_variableName,_x];
